...
def save_marks():
    ...
    try:
        ...
        rid = list_reason
        if list_reason == '0':
            db_assist.update("insert into reasons (comment) values (?)", [new_reason])
...
def reset():
    ...
    """
    reset = True
    temp = request.args.get('reset')
    if temp is not None: 
        cur = db_assist.get_single("select id from users where reset='"+temp+"'")
        if cur:
            return render_template("login.html", reset=reset, link=False)
        else:
            flash("Seems like you used invalid link. Try to reset password one more time")
            return render_template("login.html", reset=False, link=True)
    return render_template("login.html", reset=reset, link=True)
...
def reset_password():
    ...
    e = db_assist.get_single('select email,id from users where email = ?', [email])
    ...
    if user_email is not None:
        ...
        #add the pass into db
        usrid = e[1]
        db_assist.update("update users set reset = ? where id = ?",[one_time, usrid])
...
def resetting():
    ...
    if passwd is not None and confirm_passwd is not None:
        if passwd == confirm_passwd:
            temp = request.args.get('reset')
            if temp:
                cur = db_assist.get_single("select id from users where reset='"+temp+"'")
                usrid = cur if not cur else cur[0]
                if usrid:                
                    db_assist.update("update users set password = ?, reset='' where id = ?",
                                     [bcrypt.generate_password_hash(passwd), usrid])
                    flash("Your password is successfully changed. Log in with a new one")
            else:
                flash("The link you are using is not valid")
        else:
            flash("Passwords do not match")
            return render_template("login.html", reset=True, link=False)
    return redirect(url_for('log'))
