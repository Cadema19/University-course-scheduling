...
class FlaskTestCase(unittest.TestCase):
    ...
    def test_registration(self):
     ...
     # Ensure registeration behaves correctly in exception
    def test_registrationException(self):
        tester = app.test_client()
        response = tester.post(
            '/register',
            data=dict(title="mr", name="abac", username="doe", email="abcdeff@gmail.com", password="admin", confirmpass="admin", type="1"),
            follow_redirects=True
        )
        self.assertIn(b'User/Username already exists', response.data)
    ...
    def test_lecturer(self):
     ...
     # Ensure Coordinator login behaves correctly with correct credentials
    def test_Coordininator(self):
        tester = app.test_client()
        response = tester.post(
            '/login',
            data=dict(username="admin",  password="coco2015"),
            follow_redirects=True
        )
        self.assertIn(b'Logged in as: Coordinator', response.data)
    ...
    def test_loginException(self):
        ...
        self.assertIn(b'Wrong User Name!', response.data)
